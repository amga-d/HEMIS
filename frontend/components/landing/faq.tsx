        ...
        <div className="space-y-6">
          ...
          {questions.map((item, index) => (
            <div key={index} className="border-t border-gray-700 pt-5 transition-all">
              <button 
                className="w-full flex justify-between items-center text-left text-lg font-medium focus:outline-none text-blue-500 hover:text-blue-300 transition" 
              ...
              {activeIndex === index && (
                <p className="mt-3 text-gray-500">{item.answer}</p>
              )}
            </div>
          ))}
        </div>
